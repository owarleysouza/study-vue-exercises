<template>
  <div class="home">
    <ul>
      <li>
        <router-link to="/exercise-01">Exercício 01</router-link>
      </li>
      <li>
        <router-link to="/exercise-02">Exercício 02</router-link>
      </li>
      <li>
        <router-link to="/exercise-03">Exercício 03</router-link>
      </li>
      <li>
        <router-link to="/exercise-04">Exercício 04</router-link>
      </li>
      <li>
        <router-link to="/exercise-05">Exercício 05</router-link>
      </li>
      <li>
        <router-link to="/exercise-06">Exercício 06</router-link>
      </li>
      <li>
        <router-link to="/exercise-07">Exercício 07</router-link>
      </li>
      <li>
        <router-link to="/exercise-08">Exercício 08</router-link>
      </li>
      <li>
        <router-link to="/exercise-09">Exercício 09</router-link>
      </li>
      <li>
        <router-link to="/exercise-10">Exercício 10</router-link>
      </li>
      <li>
        <router-link to="/exercise-11">Exercício 11</router-link>
      </li>
      <li>
        <router-link to="/exercise-12">Exercício 12</router-link>
      </li>
      <li>
        <router-link to="/exercise-13">Exercício 13</router-link>
      </li>
    </ul>
  </div>
</template>

<script>
export default {
  name: "Home",
  components: {},
};
</script>
